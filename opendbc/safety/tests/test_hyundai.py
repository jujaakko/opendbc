...
class TestHyundaiLegacySafetyBlockFca(TestHyundaiLegacySafety):
  TX_MSGS = [[0x340, 0], [0x4F1, 0], [0x485, 0], [0x38D, 0]]
  FWD_BLACKLISTED_ADDRS = {2: [0x340, 0x485, 0x38D]}
  RELAY_MALFUNCTION_ADDRS = {0: (0x340, 0x485)}

  def setUp(self):
    self.packer = CANPackerSafety("hyundai_kia_generic")
    self.safety = libsafety_py.libsafety
    self.safety.set_safety_hooks(CarParams.SafetyModel.hyundaiLegacy, HyundaiSafetyFlags.BLOCK_FCA)
    self.safety.init_tests()

  def _fca11_msg(self, vsm_aeb_req=False, fca_aeb_req=False, aeb_decel=0):
    values = {
      "FCA_Status": 2,
  ...
  def test_block_camera_fca11_fwd(self):
    self.assertEqual(-1, self.safety.safety_fwd_hook(2, 0x38D))
    self.assertEqual(2, self.safety.safety_fwd_hook(0, 0x38D))

  def test_fca11_camera_aeb_allowed(self):
    # stock FCW/AEB bits are forwarded, not generated by openpilot
    self.assertTrue(self._tx(self._fca11_msg()))
    self.assertTrue(self._tx(self._fca11_msg(vsm_aeb_req=True)))
    self.assertTrue(self._tx(self._fca11_msg(fca_aeb_req=True)))
    self.assertTrue(self._tx(self._fca11_msg(aeb_decel=1.0)))
